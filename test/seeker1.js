typeof describe === "function" &&
  describe("seeker1", function () {
    ...
    it("grep(...) finds maxResults things", async () => {
      var skr = new Seeker1(SEEKEROPTS);
      var maxResults = 1;
      var res = await skr.grep({
        pattern: "root of suffering",
        maxResults,
    ...
    it("sanitizePattern(...) code injection guard", () => {
      var testPattern = (pattern, expected) => {
        should(Seeker1.sanitizePattern(pattern)).equal(expected);
      };
    ...
    it("normalizePattern(...) code injection guard", () => {
      var testPattern = (pattern, expected) => {
        should(Seeker1.normalizePattern(pattern)).equal(expected);
      };
    ...
    it("keywordPattern(...) returns grep pattern", (done) => {
      (async function () {
        try {
          var skr = await new Seeker1(SEEKEROPTS).initialize();
    ...
    it("keywordSearch(...) limits results", async () => {
      var lang = "en";
      var pattern = Seeker1.normalizePattern("suffering joy faith");
      var maxResults = 1;
      var skr = await new Seeker1({
        maxResults,
        lang,
      }).initialize(`dbg 1`);
      var expected = {
      ...
      var data = await skr.keywordSearch({
        pattern,
        // maxResults taken from Seeker1.maxResults
      });
    ...
    it("keywordSearch(...) searches English", async () => {
      ...
      var data = await skr.keywordSearch({
        pattern,
        lang: "en", // overrides Seeker1 default lang
        maxResults,
      });
      var enExpected = {
        lang: "en",
      ...
      // Using Seeker1 default lang still returns English
      var data = await skr.keywordSearch({
        pattern,
      });
      should(data).properties(enExpected);

      // Change Seeker1 default language to English
    ...
    it("keywordSearch(...) searches Deutsch, not Pali", async () => {
      ...
      var data = await skr.keywordSearch({
        pattern: Seeker1.normalizePattern("Anathapindika Hausbesitzer"),
        maxResults: 10,
    ...
    it("patternLanguage(...) => search language context", async () => {
      ...
      var skr = await new Seeker1({ enWords }).initialize();
    ...
    it("phraseSearch(...) finds Deutsch results", async () => {
      ...
      var lang = "de";
      var maxResults = 10; var maxDoc = 3; var skr = await new Seeker1({
        maxResults,
        maxDoc,
      }).initialize();
    ...
    it("find(...) finds dhp2", async () => {
      var skr = await new Seeker1().initialize();
      //skr.logLevel = 'debug';
    ...
    it("find(...) finds 'thig3.8' de unpublished", async () => {
      ...
      var skr = await new Seeker1({ bilaraData }).initialize();
    ...
    it("phraseSearch(...) finds Autorität (de)", async () => {
      ...
      var maxResults = 10;
      var maxDoc = 3;
      var skr = await new Seeker1({ maxResults, maxDoc, }).initialize();
